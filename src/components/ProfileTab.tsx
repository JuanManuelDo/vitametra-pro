...
import { 
  ...
  Activity, Clock, Target, Scale, X, Droplets, 
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFE] pb-40">
      <header className="relative pt-20 pb-12 px-6 bg-white rounded-b-[4rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50 blur-[100px] rounded-full -mr-32 -mt-32 opacity-60" />
        ...
        <div className="relative flex flex-col items-center">
          <motion.div 
            ...
            className="relative"
          >
            <div className="w-32 h-32 rounded-[3rem] p-1 bg-gradient-to-tr from-indigo-600 to-blue-400 shadow-2xl shadow-indigo-100">
          ...
          <h2 className="mt-8 text-4xl font-[1000] tracking-tighter text-slate-900 italic uppercase leading-none">
            {currentUser?.firstName || 'Usuario'} <span className="text-indigo-600 italic">PRO</span>
          </h2>
          
          <div className="mt-4 inline-flex items-center gap-2 px-4 py-1.5 bg-slate-900 rounded-full">
      ...
      <main className="px-6 mt-10 space-y-10">
        {/* MEMORIA IA */}
        <section className="bg-white p-8 rounded-[3rem] border border-slate-50 shadow-sm">
        ...
        <section className="space-y-6">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] ml-4">Calibración de Perfil</h3>
          <div className="bg-white rounded-[3rem] border border-slate-50 shadow-sm overflow-hidden">
            ...
            <SettingsRow 
              ...
              value={`${currentUser.targetWeight || '--'}kg • ${currentUser.targetHba1c || '--'}%`} 
              onClick={() => setActiveModal('goals')} 
            />
            <SettingsRow 
              icon={<Droplets className="text-rose-500" />} 
              label="Unidades de Medida" 
              value={currentUser.glucoseUnitPreference || 'mg/dL'} 
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between px-4">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Protocolo de Ratios</h3>
            <Scale size={16} className="text-slate-300" />
          </div>
          <div className="bg-white rounded-[3.5rem] p-4 border border-slate-50 shadow-sm">
            {ratios.map((segment, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Hora Inicio</p>
                ...
                <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-2xl border border-slate-100">
                  <input 
                    ...
                    className="w-16 bg-white border-none rounded-xl py-3 text-center font-black text-indigo-600 shadow-sm outline-none text-xl"
                  />
        ...
        <div className="space-y-4 pt-6">
            <button 
                ...
                disabled={isSaving}
                className="group relative w-full bg-slate-900 text-white p-7 rounded-[2.5rem] font-black uppercase text-[11px] tracking-[0.3em] overflow-hidden transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-indigo-100"
            >
                ...
                <span className="relative flex items-center justify-center gap-3">
                    {isSaving ? <Loader2 className="animate-spin" /> : <><ShieldCheck size={20} className="text-indigo-400" /> Sincronizar Bio-Core</>}
                </span>
            </button>

            <button 
                onClick={() => apiService.logout()}
                className="w-full flex items-center justify-center gap-3 p-6 text-rose-500 font-black text-[11px] uppercase tracking-[0.3em] rounded-[2.5rem] hover:bg-rose-50 transition-all"
            >
    ...
    >
        <div className="flex items-center gap-5">
            ...
            <div className="text-left">
                ...
                <p className="text-sm font-extrabold text-slate-800 uppercase tracking-tight italic">{value}</p>
            </div>
        </div>
